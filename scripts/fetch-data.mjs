#!/usr/bin/env node
/**
 * Refreshes the data the landing page renders from:
 *
 *   data/site.json      - upcoming + past Luma shows, recent Instagram posts, tallies
 ...
 * hop is allowed to fail, and a failed hop keeps whatever data/site.json already
...
const SCRIPT_FILE = path.join(ROOT, 'data', 'site.js');
...
async function instagramViaWeb() {
  ...
  const posts = edges
    ...
    .map(({ node: n }) => ({
      ...
      // kept only long enough to download; stripped before we write site.json
...
if (meaningful(previous) === meaningful(next)) {
  console.log('\nno change — leaving data/site.json alone');
} else {
  ...
  // Same data, as a plain script that assigns a global. The page loads this
  // rather than fetching the .json: a classic <script> is exempt from the CORS
  // rules that make fetch() fail on a file:// URL, so opening index.html
  // straight off disk shows the real shows and posts instead of nothing. The
  // .json stays as the readable copy — both are written in the same step, so
  // they can't drift.
  await writeFile(
    SCRIPT_FILE,
    '// Generated by scripts/fetch-data.mjs — do not edit by hand.\n' +
      `window.NSDS_DATA = ${json};\n`,
  );

  console.log(
    `\nwrote ${path.relative(ROOT, DATA_FILE)} + ${path.relative(ROOT, SCRIPT_FILE)} — ` +
